...
/***/ "./src/main/collectors/input-counter.ts"
/*!**********************************************!*\
  !*** ./src/main/collectors/input-counter.ts ***!
  \**********************************************/
(__unused_webpack_module, exports, __webpack_require__) {
...
const uiohook_napi_1 = __webpack_require__(/*! uiohook-napi */ "uiohook-napi");
class InputCounter {
    keys = 0;
    clicks = 0;
    scroll = 0;
    ...
    start() {
        if (this.started)
            return;
        uiohook_napi_1.uIOhook.on('keydown', () => { this.keys++; });
        uiohook_napi_1.uIOhook.on('mousedown', () => { this.clicks++; });
        uiohook_napi_1.uIOhook.on('wheel', () => { this.scroll++; });
...
/***/ "./src/main/collectors/orchestrator.ts"
/*!*********************************************!*\
  !*** ./src/main/collectors/orchestrator.ts ***!
  \*********************************************/
(__unused_webpack_module, exports, __webpack_require__) {
...
const categories_1 = __webpack_require__(/*! ../../shared/categories */ "./src/shared/categories.ts");
const filter_1 = __webpack_require__(/*! ../privacy/filter */ "./src/main/privacy/filter.ts");
const events_repo_1 = __webpack_require__(/*! ../store/events-repo */ "./src/main/store/events-repo.ts");
const config_repo_1 = __webpack_require__(/*! ../store/config-repo */ "./src/main/store/config-repo.ts");
const inputCounter = new input_counter_1.InputCounter();
let lastIdleMs = null;
...
function categorizeApp(appName, rules) {
 ...
 */
function computeAfkSec(idlePrevMs, idleNowMs, dtSec, thresholdMs) {
...
function setPaused(isPaused, pauseUntil) {
...
function collectTick() {
    ...
    lastTickTime = now;
    // Get window info
    const windowInfo = (0, win32_1.getForegroundWindowInfo)();
    const app = windowInfo?.app ?? '';
    const title = windowInfo?.title ?? '';
    // Categorize
    const category = categorizeApp(app, categories_1.DEFAULT_CATEGORIES);
    // Get idle time
    const idleMs = (0, win32_1.getIdleTimeMs)();
    ...
    const filtered = (0, filter_1.applyPrivacyFilter)(rawEvent, config);
    // Store in SQLite
    (0, events_repo_1.insertEvent)(filtered);
    // Update today totals
    const totals = (0, events_repo_1.getTodayTotals)();
    const todayTotals = {
        keys: totals.keys,
...
exports.getIdleTimeMs = getIdleTimeMs;
...
const HANDLE = koffi_1.default.pointer('HANDLE', koffi_1.default.opaque());
const DWORD = koffi_1.default.types.uint32;
const UINT = koffi_1.default.types.uint32;
const BOOL = koffi_1.default.types.int32;
const LASTINPUTINFO = koffi_1.default.struct('LASTINPUTINFO', {
    cbSize: UINT,
    dwTime: DWORD,
});
// DLL bindings
...
const GetWindowTextW = user32.func('GetWindowTextW', 'int', [HWND, koffi_1.default.out(koffi_1.default.pointer(koffi_1.default.types.uint16)), 'int']);
const GetWindowThreadProcessId = user32.func('GetWindowThreadProcessId', DWORD, [HWND, koffi_1.default.out(koffi_1.default.pointer(DWORD))]);
const GetLastInputInfo = user32.func('GetLastInputInfo', BOOL, [koffi_1.default.inout(koffi_1.default.pointer(LASTINPUTINFO))]);
...
const QueryFullProcessImageNameW = kernel32.func('QueryFullProcessImageNameW', BOOL, [HANDLE, DWORD, koffi_1.default.out(koffi_1.default.pointer(koffi_1.default.types.uint16)), koffi_1.default.inout(koffi_1.default.pointer(DWORD))]);
...
exports.forwardStateUpdates = forwardStateUpdates;
exports.forwardSyncUpdates = forwardSyncUpdates;
const electron_1 = __webpack_require__(/*! electron */ "electron");
const ipc_channels_1 = __webpack_require__(/*! ../../shared/ipc-channels */ "./src/shared/ipc-channels.ts");
const orchestrator_1 = __webpack_require__(/*! ../collectors/orchestrator */ "./src/main/collectors/orchestrator.ts");
const config_repo_1 = __webpack_require__(/*! ../store/config-repo */ "./src/main/store/config-repo.ts");
const events_repo_1 = __webpack_require__(/*! ../store/events-repo */ "./src/main/store/events-repo.ts");
const markers_repo_1 = __webpack_require__(/*! ../store/markers-repo */ "./src/main/store/markers-repo.ts");
const sync_worker_1 = __webpack_require__(/*! ../sync/sync-worker */ "./src/main/sync/sync-worker.ts");
const filter_1 = __webpack_require__(/*! ../privacy/filter */ "./src/main/privacy/filter.ts");
let mainWindow = null;
...
function registerIpcHandlers() {
    // State queries
    electron_1.ipcMain.handle(ipc_channels_1.IPC.STATE_GET, () => {
        return (0, orchestrator_1.getState)();
    });
    electron_1.ipcMain.handle(ipc_channels_1.IPC.CONFIG_GET, () => {
        const config = (0, config_repo_1.getAllConfig)();
        // SECURITY: mask deviceKey before sending to renderer
        return { ...config, 'server.deviceKey': config['server.deviceKey'] ? '***' : '' };
    });
    electron_1.ipcMain.handle(ipc_channels_1.IPC.SYNC_STATUS, () => {
        return (0, sync_worker_1.getSyncStatus)();
    });
    electron_1.ipcMain.handle(ipc_channels_1.IPC.TIMELINE_GET, (_event, from, to) => {
        return (0, events_repo_1.getEventsInRange)(from, to);
    });
    electron_1.ipcMain.handle(ipc_channels_1.IPC.MARKERS_GET, (_event, from, to) => {
        return (0, markers_repo_1.getMarkersInRange)(from, to);
    });
    electron_1.ipcMain.handle(ipc_channels_1.IPC.PRIVACY_PREVIEW, () => {
        const state = (0, orchestrator_1.getState)();
        if (!state.now)
            return null;
        const config = (0, config_repo_1.getAllConfig)();
        return (0, filter_1.previewFilter)({
            app: state.now.app,
    ...
    // Config mutations
    electron_1.ipcMain.handle(ipc_channels_1.IPC.CONFIG_UPDATE, (_event, partial) => {
    ...
    electron_1.ipcMain.handle(ipc_channels_1.IPC.PAUSE, (_event, minutes) => {
        ...
        (0, orchestrator_1.setPaused)(true, pauseUntil);
        sendToRenderer(ipc_channels_1.IPC.PAUSE_CHANGED, { isPaused: true, pauseUntil });
    });
    electron_1.ipcMain.handle(ipc_channels_1.IPC.RESUME, () => {
        (0, orchestrator_1.setPaused)(false, null);
        sendToRenderer(ipc_channels_1.IPC.PAUSE_CHANGED, { isPaused: false, pauseUntil: null });
    });
    // Markers
    electron_1.ipcMain.handle(ipc_channels_1.IPC.MARKER_ADD, (_event, marker) => {
        const state = (0, orchestrator_1.getState)();
        (0, markers_repo_1.insertMarker)(marker.type, marker.note, state.now?.app, state.category);
    });
    // Blacklist management
    electron_1.ipcMain.handle(ipc_channels_1.IPC.BLACKLIST_ADD_APP, (_event, app) => {
        const config = (0, config_repo_1.getAllConfig)();
        const list = [...config['privacy.blacklistApps']];
        if (!list.includes(app)) {
            list.push(app);
            (0, config_repo_1.updateConfig)({ 'privacy.blacklistApps': list });
        }
    });
    electron_1.ipcMain.handle(ipc_channels_1.IPC.BLACKLIST_REMOVE_APP, (_event, app) => {
        const config = (0, config_repo_1.getAllConfig)();
        const list = config['privacy.blacklistApps'].filter(a => a !== app);
        (0, config_repo_1.updateConfig)({ 'privacy.blacklistApps': list });
    });
    electron_1.ipcMain.handle(ipc_channels_1.IPC.BLACKLIST_ADD_PATTERN, (_event, pattern) => {
        const config = (0, config_repo_1.getAllConfig)();
        const list = [...config['privacy.blacklistTitlePatterns']];
        if (!list.includes(pattern)) {
            list.push(pattern);
            (0, config_repo_1.updateConfig)({ 'privacy.blacklistTitlePatterns': list });
        }
    });
    electron_1.ipcMain.handle(ipc_channels_1.IPC.BLACKLIST_REMOVE_PATTERN, (_event, pattern) => {
        const config = (0, config_repo_1.getAllConfig)();
        const list = config['privacy.blacklistTitlePatterns'].filter(p => p !== pattern);
        (0, config_repo_1.updateConfig)({ 'privacy.blacklistTitlePatterns': list });
    });
    // Sync
    electron_1.ipcMain.handle(ipc_channels_1.IPC.SYNC_NOW, async () => {
        await (0, sync_worker_1.syncNow)();
    });
    electron_1.ipcMain.handle(ipc_channels_1.IPC.SYNC_TEST, async () => {
        return await (0, sync_worker_1.testConnectivity)();
    });
    // Device setup
    electron_1.ipcMain.handle(ipc_channels_1.IPC.SETUP_DEVICE, (_event, config) => {
        (0, config_repo_1.updateConfig)({
            'server.baseUrl': config.baseUrl,
            'server.deviceId': config.deviceId,
            'server.deviceKey': config.deviceKey,
        });
    });
    electron_1.ipcMain.handle(ipc_channels_1.IPC.IS_CONFIGURED, () => {
        return (0, config_repo_1.isConfigured)();
    });
}
// Forward collector state updates to renderer
function forwardStateUpdates() {
    const { onStateUpdate } = __webpack_require__(/*! ../collectors/orchestrator */ "./src/main/collectors/orchestrator.ts");
    onStateUpdate((state) => {
        sendToRenderer(ipc_channels_1.IPC.STATE_UPDATED, state);
    });
}
// Forward sync status updates to renderer
function forwardSyncUpdates() {
    const { onSyncUpdate } = __webpack_require__(/*! ../sync/sync-worker */ "./src/main/sync/sync-worker.ts");
    onSyncUpdate((status) => {
        sendToRenderer(ipc_channels_1.IPC.SYNC_UPDATED, status);
    });
}
...
exports.previewFilter = previewFilter;
const PRIVACY_PLACEHOLDER = '[PRIVACY]';
function isBlacklistedApp(app, blacklist) {
    const lower = app.toLowerCase();
    return blacklist.some(b => lower.includes(b.toLowerCase()));
}
function isBlacklistedTitle(title, patterns) {
...
function shouldRedactDomain(title, domains) {
    return domains.some(domain => title.toLowerCase().includes(domain.toLowerCase()));
}
function applyPrivacyFilter(event, config) {
    const result = {
        ...event,
 ...
 */
function previewFilter(event, config) {
...
const database_1 = __webpack_require__(/*! ./database */ "./src/main/store/database.ts");
const types_1 = __webpack_require__(/*! ../../shared/types */ "./src/shared/types.ts");
function getConfigValue(key) {
    const db = (0, database_1.getDb)();
    const row = db.prepare('SELECT value FROM config WHERE key = ?').get(key);
    if (!row)
        return types_1.DEFAULT_CONFIG[key];
    const defaultVal = types_1.DEFAULT_CONFIG[key];
...
function getAllConfig() {
    const result = { ...types_1.DEFAULT_CONFIG };
    const db = (0, database_1.getDb)();
    const rows = db.prepare('SELECT key, value FROM config').all();
    for (const row of rows) {
        const key = row.key;
        if (!(key in types_1.DEFAULT_CONFIG))
            continue;
        const defaultVal = types_1.DEFAULT_CONFIG[key];
        if (typeof defaultVal === 'number') {
...
function getDb() {
    if (db)
        return db;
    const dbPath = path_1.default.join(electron_1.app.getPath('userData'), 'activity.db');
...
exports.markEventsFailed = markEventsFailed;
exports.resetFailedEvents = resetFailedEvents;
exports.getQueuedCount = getQueuedCount;
exports.getFailedCount = getFailedCount;
exports.getTodayTotals = getTodayTotals;
...
function insertEvent(event) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
    const result = stmt.run(event.app, event.windowTitle, event.category, event.isAfk ? 1 : 0, event.keys, event.clicks, event.scroll, event.dtSec, event.activeSec, event.afkSec, event.idleMs);
...
function getUnsyncedEvents(limit) {
    const db = (0, database_1.getDb)();
    const stmt = db.prepare(`
    SELECT id, created_at as createdAt, app, window_title as windowTitle,
           category, is_afk as isAfk, keys, clicks, scroll,
...
function markEventsFailed(ids, error) {
    if (ids.length === 0)
        return;
    const db = (0, database_1.getDb)();
    const placeholders = ids.map(() => '?').join(',');
...
function resetFailedEvents() {
    const db = (0, database_1.getDb)();
    db.prepare(`UPDATE events SET synced = 0 WHERE synced = 2 AND sync_attempts < 5`).run();
}
function getQueuedCount() {
    const db = (0, database_1.getDb)();
    const row = db.prepare(`SELECT COUNT(*) as cnt FROM events WHERE synced = 0`).get();
    return row.cnt;
}
function getFailedCount() {
    const db = (0, database_1.getDb)();
    const row = db.prepare(`SELECT COUNT(*) as cnt FROM events WHERE synced = 2`).get();
    return row.cnt;
}
...
function purgeOldEvents(retentionDays) {
    const db = (0, database_1.getDb)();
...
/***/ "./src/main/store/markers-repo.ts"
/*!****************************************!*\
  !*** ./src/main/store/markers-repo.ts ***!
  \****************************************/
(__unused_webpack_module, exports, __webpack_require__) {
...
exports.getMarkersInRange = getMarkersInRange;
exports.getUnsyncedMarkers = getUnsyncedMarkers;
exports.markMarkersSynced = markMarkersSynced;
exports.markMarkersFailed = markMarkersFailed;
const database_1 = __webpack_require__(/*! ./database */ "./src/main/store/database.ts");
function insertMarker(type, note, appContext, categoryContext) {
    const db = (0, database_1.getDb)();
    const stmt = db.prepare(`
...
function getMarkersInRange(from, to) {
    const db = (0, database_1.getDb)();
    const stmt = db.prepare(`
...
function getUnsyncedMarkers(limit) {
    const db = (0, database_1.getDb)();
    const stmt = db.prepare(`
    SELECT id, created_at as createdAt, type, note,
...
function markMarkersSynced(ids) {
    if (ids.length === 0)
        return;
    const db = (0, database_1.getDb)();
    const placeholders = ids.map(() => '?').join(',');
...
function markMarkersFailed(ids, error) {
    if (ids.length === 0)
        return;
    const db = (0, database_1.getDb)();
    const placeholders = ids.map(() => '?').join(',');
...
/***/ "./src/main/store/sync-state-repo.ts"
/*!*******************************************!*\
  !*** ./src/main/store/sync-state-repo.ts ***!
  \*******************************************/
(__unused_webpack_module, exports, __webpack_require__) {
...
exports.setLastSyncResult = setLastSyncResult;
exports.getConsecutiveFailures = getConsecutiveFailures;
exports.setConsecutiveFailures = setConsecutiveFailures;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.onSyncUpdate = onSyncUpdate;
...
const markers_repo_1 = __webpack_require__(/*! ../store/markers-repo */ "./src/main/store/markers-repo.ts");
const config_repo_1 = __webpack_require__(/*! ../store/config-repo */ "./src/main/store/config-repo.ts");
const sync_state_repo_1 = __webpack_require__(/*! ../store/sync-state-repo */ "./src/main/store/sync-state-repo.ts");
...
function getSyncStatus() {
    const config = (0, config_repo_1.getAllConfig)();
    return {
        lastSyncAt: (0, sync_state_repo_1.getLastSyncAt)(),
        lastSyncResult: (0, sync_state_repo_1.getLastSyncResult)(),
        queuedCount: (0, events_repo_1.getQueuedCount)(),
        failedCount: (0, events_repo_1.getFailedCount)(),
        isOnline,
        deviceId: config['server.deviceId'],
        consecutiveFailures: (0, sync_state_repo_1.getConsecutiveFailures)(),
    };
}
...
async function syncBatch() {
    ...
    try {
        ...
        // Sync events
        const events = (0, events_repo_1.getUnsyncedEvents)(batchSize);
        if (events.length > 0) {
            ...
            try {
                await axios_1.default.post(`${baseUrl}/api/activity/v2/artifacts/batch`, {
                    artifacts,
                }, {
                    headers: {
                ...
                (0, events_repo_1.markEventsSynced)(ids);
                isOnline = true;
                (0, sync_state_repo_1.resetConsecutiveFailures)();
            }
            catch (err) {
                const ids = events.map(e => e.id);
                const errorMsg = err instanceof Error ? err.message : String(err);
                (0, events_repo_1.markEventsFailed)(ids, errorMsg);
                isOnline = false;
                (0, sync_state_repo_1.setConsecutiveFailures)((0, sync_state_repo_1.getConsecutiveFailures)() + 1);
            }
        }
        // Sync markers
        const markers = (0, markers_repo_1.getUnsyncedMarkers)(batchSize);
        if (markers.length > 0) {
            for (const marker of markers) {
                try {
                    await axios_1.default.post(`${baseUrl}/api/activity/v2/markers`, {
                    ...
                    (0, markers_repo_1.markMarkersSynced)([marker.id]);
                    isOnline = true;
                }
                catch (err) {
                    const errorMsg = err instanceof Error ? err.message : String(err);
                    (0, markers_repo_1.markMarkersFailed)([marker.id], errorMsg);
                    isOnline = false;
                }
            }
        }
        ...
        if (syncedAny) {
            (0, sync_state_repo_1.setLastSyncAt)(new Date().toISOString());
            (0, sync_state_repo_1.setLastSyncResult)(isOnline ? 'ok' : 'partial_failure');
        }
        // Periodically reset failed events for retry
        (0, events_repo_1.resetFailedEvents)();
        // Purge old synced events (30-day retention)
        (0, events_repo_1.purgeOldEvents)(RETENTION_DAYS);
        notifyStatus();
    }
    finally {
        isSyncing = false;
...
function getBackoffMs() {
    const failures = (0, sync_state_repo_1.getConsecutiveFailures)();
    if (failures <= 0)
        return 0;
...
async function testConnectivity() {
    ...
    try {
        await axios_1.default.get(`${baseUrl}/api/activity/v2/timeline`, {
...
exports.updateTrayTooltip = updateTrayTooltip;
const electron_1 = __webpack_require__(/*! electron */ "electron");
let tray = null;
/**
...
function createMainWindow() {
    mainWindow = new electron_1.BrowserWindow({
        ...
        resizable: true,
        webPreferences: {
            preload: 'C:\\Projects\\rodion.pro\\activity-desktop\\.webpack\\renderer\\main_window\\preload.js',
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.MARKER_PRESETS = exports.DEFAULT_CONFIG = void 0;
...
"use strict";
module.exports = require("uiohook-napi");
...
const database_1 = __webpack_require__(/*! ./store/database */ "./src/main/store/database.ts");
...
electron_1.app.on('ready', () => {
    ...
    (0, ipc_handlers_1.setMainWindow)(mainWindow);
    // Create system tray
    (0, tray_1.createTray)(mainWindow);
    // Defer native module initialization to avoid blocking window render
    setTimeout(() => {
        try {
            const { startCollecting } = __webpack_require__(/*! ./collectors/orchestrator */ "./src/main/collectors/orchestrator.ts");
            startCollecting();
        ...
        try {
            const { startSyncWorker } = __webpack_require__(/*! ./sync/sync-worker */ "./src/main/sync/sync-worker.ts");
            startSyncWorker();
        ...
        try {
            (0, ipc_handlers_1.forwardStateUpdates)();
            (0, ipc_handlers_1.forwardSyncUpdates)();
        }
        catch (err) {
...
electron_1.app.on('will-quit', () => {
    try {
        const { stopCollecting } = __webpack_require__(/*! ./collectors/orchestrator */ "./src/main/collectors/orchestrator.ts");
        stopCollecting();
    }
    catch { /* ignore */ }
    try {
        const { stopSyncWorker } = __webpack_require__(/*! ./sync/sync-worker */ "./src/main/sync/sync-worker.ts");
        stopSyncWorker();
    }
    catch { /* ignore */ }
